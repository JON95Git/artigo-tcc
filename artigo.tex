...
apenas as implementações e código dependente de plataforma. 
...
dentro desse limite anteriomente definido.
...
os testes, por isso recomenda-se que se defina uma lista de testes como a seguinte:

\begin{itemize}
\item Checar sanidade do RNG (dados não devem se repetir com frequência)
...
\item Checar se os limites de mínimo e máixmo são respeitados (mínimo deve ser menor que máixmo)
\item Checar se o numero dado realmente está no range exigido
\end{itemize}
...
A prática da escrita de código dessa maneira é chamada de \textit{Dual-targeting}.
