...
\textit{Resumo -} \normalfont{\textit{Este artigo apresenta técnicas e ferramentas
    ...
    Foi utilizada uma aplicação de exemplo, uma interface gráfica para requisição de amostras de números verdadeiramente aleatórios (TRNG - True Random Number Generator), capaz de rodar em ambiente desktop e embarcado.}}
\end {abstract}
...
\textit{Abstract - } \normalfont{\textit{This article presents techniques and tools
    ...
    An example application was used, a graphical interface for requesting samples of truly random numbers (TRNG - True Random Number Generator), capable of running in both desktop and embedded environments.}}
\end {abstract}
\noindent\begin{keywords}
...
\itemsep0em 
  ...
  \item \textit{Testes de integração} - testam a integração entre módulos de \textit{software}
  \item \textit{Testes unitários} - testes de unidade, testam a menor unidade de
        código testável
\end{itemize}
...
Foram utilizadas as seguintes ferramentas de \textit{software} na execução do projeto:
...
A metodologia utilizada para o desenvolvimento do projeto seguiu a técnica de
\textit{TDD} descrita anteriormente.

Nas três etapas (\textit{Teste falha}, \textit{Teste passa}
e \textit{Refatora}) foram utilizadas ferramentas e mecanismos de \textit{software}
...
Essa interface gráfica é capaz de ser executada em dois ambientes 
diferentes:embarcado e \textit{desktop}. 
As funcionalidades, interfaces de aplicação (\textit{API - 
application programming interface}) e visual são exatamente os mesmos, variando-se
apenas as implementações de código dependente de plataforma. 

Existem duas abas disponíveis para o usuário, a primeira, chamada de \textit{Mass 
storage}, permite que seja salvo um arquivo no sistema de arquivos de 
tamanho e nomes a serem escolhidos pelo usuário. Essa aba contém uma caixa de texto 
para receber o nome do arquivo, um \textit{spinbox} para se definir o tamanho do 
arquivo, e um botão de salvar.

A segunda aba chamada de \textit{Random}, permite que o usuário escolha um limite
mínimo e máximo, e ao clicar em \textit{Get random}, irá obter um número aleatório
dentro desse limite anteriomente definido. Essa aba contém dois \textit{spinbox}
...
extensão \textit{.h}), variando-se apenas a implementação dessas interfaces nos
...
diretamente a esses componentes, mas sim as suas abstrações. A prática da escrita de 
código dessa maneira é chamada de \textit{Dual-targeting}.
...
\begin{lstlisting}[language=bash]
$ ceedling module:create[rng_user]
\end{lstlisting}
...
\begin{figure}[H]
  \centering
  \caption{Arquivo de testes inicial - \textit{test\_rng\_user.c}}
\begin{lstlisting}[language=C]
#include "unity.h"
#include "rng_user.h"
...
void test_rng__userNeedToImplement(void)
{
...
\begin{figure}[H]
  \centering
  \caption{Arquivo de testes com um teste simples  - \textit{test\_rng\_user.c}}
\begin{lstlisting}[language=C]
#include "unity.h"
#include "rng_user.h"
...
O código da aplicação resultante da utilização das técnicas e ferramentas 
apresentadas é de fácil compreensão, manutenção e portabilidade, como pode ser visto
na Figura \ref{fig:app-code}.\hfill\

\begin{figure}[H]
  \centering
  \caption{Trecho de código da aplicação}
\begin{lstlisting}[language=C]
#include "app_main.h"
...
O código de teste resultante também apresenta uma estrutura simples
e escalável, como pode ser visto na Figura \ref{fig:test-code}.\hfill\

\begin{figure}[H]
  \centering
  \caption{Trecho de código de teste}
\begin{lstlisting}[language=C]
...
\begin{figure}[H]
  \centering
  \caption{Cobertura de código da aplicação}
  \includegraphics[width=1\linewidth]{images/code-coverage-2.png}
  \caption*{\newline\textbf{Fonte:} acervo do autor}
  \label{fig:code-coverage}
\end{figure}

É possível navegar por cada arquivo de código fonte e inspecionar os resultados de
forma mais detalhada como é apresentado na Figura \ref{fig:code-coverage-detail}:

\begin{figure}[H]
  \centering
  \caption{Cobertura detalhada de código da aplicação}
  \includegraphics[width=1\linewidth]{images/code-coverage-detail.png}
  \caption*{\newline\textbf{Fonte:} acervo do autor}
  \label{fig:code-coverage-detail}
\end{figure}

A relação entre os testes, o tempo de execução e o \textit{status} de execução dos
mesmos pode ser visto na Figura \ref{fig:table-tests}:

\begin{figure}[H]
  \centering
  \caption{Tabela contendo dados de execução dos testes}
  \includegraphics[width=1\linewidth]{images/table-tests.png}
  \caption*{\newline\textbf{Fonte:} acervo do autor}
  \label{fig:table-tests}
\end{figure}
